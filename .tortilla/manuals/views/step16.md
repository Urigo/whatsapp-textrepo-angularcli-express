...
#### [Step 6.1: TypeORM with PostgreSQL](https://github.com/Urigo/whatsapp-textrepo-angularcli-express/commit/0484a56)
...
-┊ 98┊   ┊        const messages = chat.messages.reduce<Message[]>((filtered, message) => {
...
-┊133┊   ┊        const messages = chat.messages.reduce<Message[]>((filtered, message) => {
...
+┊   ┊318┊      }
+┊   ┊319┊
 ┊280┊320┊      let deletedIds: number[] = [];
-┊281┊   ┊      chats = chats.map(chat => {
-┊282┊   ┊        if (chat.id === Number(chatId)) {
-┊283┊   ┊          // Instead of chaining map and filter we can loop once using reduce
-┊284┊   ┊          const messages = chat.messages.reduce<Message[]>((filtered, message) => {
-┊285┊   ┊            if (all || messageIds!.includes(message.id)) {
-┊286┊   ┊              deletedIds.push(message.id);
...
+┊   ┊325┊        if (all || messageIds!.includes(message.id)) {
+┊   ┊326┊          deletedIds.push(message.id);
...
-┊307┊   ┊        ({messageAdded}: {messageAdded: Message & {chat: {id: number}}}, {chatId}: MessageAddedSubscriptionArgs, {user: currentUser}: { user: User }) => {
...
-┊309┊   ┊            !!messageAdded.recipients.find((recipient: Recipient) => recipient.userId === currentUser.id);
...
-┊314┊   ┊        ({creatorId, chatAdded}: {creatorId: string, chatAdded: Chat}, variables: any, {user: currentUser}: { user: User }) => {
...
-┊332┊   ┊      .sort((a, b) => b.createdAt - a.createdAt) || <Message[]>[];
...
| [< Previous Step](https://github.com/Urigo/whatsapp-textrepo-angularcli-express/tree/master@1.3.1/.tortilla/manuals/views/step15.md) |
|:----------------------|
